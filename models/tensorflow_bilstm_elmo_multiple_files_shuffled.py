...
VECTOR_DIM = 1024 #elmo
#VECTOR_DIM = 768 # bert
MAX_SEQUENCE_LEN = 30
IN_FILENAMES = ['./vectors_with_classes_elmo_avg_slo_1_expressions.txt',
                './vectors_with_classes_elmo_avg_slo_2_expressions.txt',
                './vectors_with_classes_elmo_avg_slo_3_expressions.txt']
...
with open('./test_results_elmo_shuffled.txt', 'w', encoding='utf-8') as outf:
    ...
    for k, i in dbe.items():
        ...
        all_X = np.array([x[0] for x in all_data])
        all_Y = [x[1] for x in all_data]
        train_X, test_X, train_Y, test_Y = train_test_split(all_X, all_Y, test_size=0.30, shuffle=True)
